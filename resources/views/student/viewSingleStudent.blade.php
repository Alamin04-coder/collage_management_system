...
<body>
  ...
  <div class="profile-card text-center">
    @if($student->image)
    <img src="{{ asset('student_images/'.$student->image) }}" alt="Profile Image" class="profile-img">
    @else
    <img src="{{ asset('images/logo2.png') }}" alt="Default Image" class="profile-img">
    @endif

    <div class="profile-info text-start">
      <p><span class="profile-title">Name:</span> {{$student->name ?? 'not found'}}</p>
      <p><span class="profile-title">Session:</span> {{$student->session ?? 'not found'}}</p>
      <p><span class="profile-title">Department:</span> {{$student->department ?? 'not found'}}</p>
      <p><span class="profile-title">Class Roll:</span> {{$student->class_roll ?? 'not found'}}</p>
    ...
    <div>
      <a href="{{ url()->previous()}}" class="btn btn-custom mt-3">⬅ Back</a>
    </div>
  </div>
